...
/**
 * 修改 basedpyright 扩展的 extension.js，将 ms-python.python 替换为 senweaver.python-environment
 * 这样 basedpyright 就可以使用我们的 python-environment 扩展来获取 Python 路径
 */
function patchBasedPyrightExtensionJs(input) {
    const jsFilter = (0, gulp_filter_1.default)('**/dist/extension.js', { restore: true });
    return input
        .pipe(jsFilter)
        .pipe((0, gulp_buffer_1.default)())
        .pipe(event_stream_1.default.mapSync((f) => {
        let contents = f.contents.toString('utf8');
        // 替换 ms-python.python 为 senweaver.python-environment
        contents = contents.replace(/ms-python\.python/g, 'senweaver.python-environment');
...
function packageMarketplaceExtensionsStream(forWeb) {
    ...
    const marketplaceExtensionsStream = minifyExtensionResources(event_stream_1.default.merge(...marketplaceExtensionsDescriptions
        .map(extension => {
        let src = (0, builtInExtensions_1.getExtensionStream)(extension).pipe((0, gulp_rename_1.default)(p => p.dirname = `extensions/${p.dirname}`));
        // 为 Vue 扩展注入 typescript.js shim 文件
        if (extension.name === 'vue.volar') {
            const typescriptShim = new vinyl_1.default({
                path: 'extensions/vue.volar/dist/typescript.js',
                contents: Buffer.from('module.exports = require("../../../node_modules/typescript/lib/typescript.js");')
            });
            src = event_stream_1.default.merge(src, event_stream_1.default.readArray([typescriptShim]));
        }
        // 修改 basedpyright 扩展，使其使用 senweaver.python-environment 而不是 ms-python.python
        if (extension.name === 'detachhead.basedpyright') {
            src = patchBasedPyrightExtensionJs(src);
        }
        return updateExtensionPackageJSON(src, (data) => {
            delete data.scripts;
            delete data.dependencies;
            delete data.devDependencies;
            // 修改 basedpyright 扩展的默认配置
            if (data.name === 'basedpyright' && data.contributes?.configuration?.properties) {
                const props = data.contributes.configuration.properties;
                // 使用内置版本避免依赖 ms-python 扩展
                if (props['basedpyright.importStrategy']) {
                    props['basedpyright.importStrategy'].default = 'useBundled';
                }
                // 将类型检查模式设为 basic，减少黄色警告
                if (props['basedpyright.analysis.typeCheckingMode']) {
                    props['basedpyright.analysis.typeCheckingMode'].default = 'basic';
                }
                // 关闭循环导入检测警告
                if (props['basedpyright.analysis.diagnosticSeverityOverrides']) {
                    props['basedpyright.analysis.diagnosticSeverityOverrides'].default = {
            ...
            // 修改 Vue (Volar) 扩展的激活事件，改为仅在打开 .vue 文件时激活
            if (data.name === 'volar' && data.activationEvents) {
                // 只在用户打开 .vue 文件时激活，避免工作区扫描导致提前激活
                data.activationEvents = ['onLanguage:vue'];
            }
            return data;
        });
    })));
