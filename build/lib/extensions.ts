 ...
 */
function patchBasedPyrightExtensionJs(input: Stream): Stream {
	const jsFilter = filter('**/dist/extension.js', { restore: true });
	return input
		.pipe(jsFilter)
		.pipe(buffer())
		.pipe(es.mapSync((f: File) => {
	...
	const marketplaceExtensionsStream = minifyExtensionResources(
		es.merge(
			...marketplaceExtensionsDescriptions
				.map(extension => {
					let src = getExtensionStream(extension).pipe(rename(p => p.dirname = `extensions/${p.dirname}`));
					// 为 Vue 扩展注入 typescript.js shim 文件
					if (extension.name === 'vue.volar') {
						const typescriptShim = new File({
