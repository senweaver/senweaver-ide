 ...
 */
function patchBasedPyrightExtension() {
    const basedpyrightPath = path_1.default.join(root, '.build/builtInExtensions/detachhead.basedpyright');
    // 检查扩展是否存在
    if (!fs_1.default.existsSync(basedpyrightPath)) {
        return;
    }
    // 1. 修改 package.json
    const packageJsonPath = path_1.default.join(basedpyrightPath, 'package.json');
    if (fs_1.default.existsSync(packageJsonPath)) {
        try {
            let content = fs_1.default.readFileSync(packageJsonPath, 'utf8');
            let modified = false;
            ...
            if (typeCheckingModeRegex.test(content)) {
                content = content.replace(/("basedpyright\.analysis\.typeCheckingMode"[\s\S]*?"default":\s*)"[^"]*"/, '$1"basic"');
                modified = true;
            }
            if (modified) {
                fs_1.default.writeFileSync(packageJsonPath, content, 'utf8');
                log(ansi_colors_1.default.blue('[patch]'), 'basedpyright package.json', ansi_colors_1.default.green('✔︎'));
            }
        }
        catch (err) {
            log(ansi_colors_1.default.red('[patch]'), `Failed to patch basedpyright package.json: ${err}`);
        }
    }
    // 2. 修改 extension.js，将 ms-python.python 替换为 senweaver.python-environment
    const extensionJsPath = path_1.default.join(basedpyrightPath, 'dist/extension.js');
    if (fs_1.default.existsSync(extensionJsPath)) {
        try {
            let content = fs_1.default.readFileSync(extensionJsPath, 'utf8');
            if (content.includes('ms-python.python')) {
                content = content.replace(/ms-python\.python/g, 'senweaver.python-environment');
                fs_1.default.writeFileSync(extensionJsPath, content, 'utf8');
                log(ansi_colors_1.default.blue('[patch]'), 'basedpyright extension.js (ms-python.python -> senweaver.python-environment)', ansi_colors_1.default.green('✔︎'));
            }
        }
        catch (err) {
            log(ansi_colors_1.default.red('[patch]'), `Failed to patch basedpyright extension.js: ${err}`);
        }
    }
}
