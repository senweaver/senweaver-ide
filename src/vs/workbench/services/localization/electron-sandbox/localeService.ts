...
class NativeLocaleService implements ILocaleService {
	...
	async setLocale(languagePackItem: ILanguagePackItem, skipDialog = false): Promise<void> {
		...
		if (locale === currentLocale || (!locale && Language.isDefaultVariant())) {
			return;
		}
		...
		try {
			...
			if (!installedLanguages.some(installedLanguage => installedLanguage.id === locale)) {
	...
	private async showRestartDialog(languageName: string): Promise<boolean> {
		const { confirmed } = await this.dialogService.confirm({
			type: 'info',
			message: localize('restartDisplayLanguageMessage1', "To change the display language, {0} needs to reload", this.productService.nameLong),
