...
export class WebLocaleService implements ILocaleService {
	...
	async setLocale(languagePackItem: ILanguagePackItem, skipRestart = false): Promise<void> {
		const locale = languagePackItem.id;
		const currentLanguage = Language.value();

		if (locale === currentLanguage || (!locale && currentLanguage === navigator.language.toLowerCase())) {
			return;
		}
		if (locale) {
		...
		// Always show restart dialog for language changes
