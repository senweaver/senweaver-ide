...
export class WorkbenchThemeService extends Disposable implements IWorkbenchThemeService {
	...
	constructor(
		...
		extensionService.whenInstalledExtensionsRegistered().then(_ => {
			...
			this.initialize().then(async () => {
				// Critical fix: For light themes on Electron, force repaint to fix invisible text issue
				// Root cause: Electron 34.3.2 has a rendering bug where light theme text doesn't paint initially
				// Solution: Toggle display:none/block on workbench to force complete re-render
				if (this.currentColorTheme.type === ColorScheme.LIGHT && !isWeb) {
					const forceRepaint = () => {
						try {
							const workbench = mainWindow.document.querySelector('.monaco-workbench') as HTMLElement;
							if (workbench) {
								// Force repaint by toggling display (most reliable method)
								const originalDisplay = workbench.style.display;
								workbench.style.display = 'none';
								void workbench.offsetHeight; // Force reflow
								workbench.style.display = originalDisplay || '';
								void workbench.offsetHeight; // Force second reflow

								// Also trigger layout service
								this.layoutService.layout();
							}
						} catch (error) {
							// Silently fail - this is a workaround for a rendering bug
							console.error('[Theme] Light theme repaint failed:', error);
						}
					};

					// First repaint after initial render
					setTimeout(forceRepaint, 1500);

					// Second repaint to catch lazy-loaded panels (bottom panel, sidebar, etc.)
					setTimeout(forceRepaint, 3000);

					// Backup: Add one-time mousemove listener to fix panels that render on hover
					let hasTriggeredMouseFix = false;
					const mouseFixHandler = () => {
						if (!hasTriggeredMouseFix) {
							hasTriggeredMouseFix = true;
							setTimeout(forceRepaint, 100); // Small delay to let panel render
							mainWindow.document.removeEventListener('mousemove', mouseFixHandler);
						}
					};
					mainWindow.document.addEventListener('mousemove', mouseFixHandler, { once: false });
	...
	private initialize(): Promise<[IWorkbenchColorTheme | null, IWorkbenchFileIconTheme | null, IWorkbenchProductIconTheme | null]> {
		...
		const initializeColorTheme = async () => {
			...
			const result = await this.setColorTheme(theme && theme.id, undefined);

			// Critical fix: For light themes, force theme change event after full load
			// This ensures all UI components receive the update notification
			if (result && result.type === ColorScheme.LIGHT) {
				setTimeout(() => {
					this.onColorThemeChange.fire(this.currentColorTheme);

					// Force aggressive repaint for light themes using multiple techniques
					const workbench = mainWindow.document.querySelector('.monaco-workbench') as HTMLElement;
					if (workbench) {
						// Method 1: Force style recalculation via transform
						const originalTransform = workbench.style.transform;
						workbench.style.transform = 'translateZ(0)';
						void workbench.offsetHeight;

						// Method 2: Force paint via opacity
						const originalOpacity = workbench.style.opacity;
						workbench.style.opacity = '0.99';
						void workbench.offsetHeight;

						// Restore and force final reflow
						workbench.style.transform = originalTransform;
						workbench.style.opacity = originalOpacity;
						void workbench.offsetHeight;

						// Method 3: Force all child elements to recalculate
						const parts = workbench.querySelectorAll('.part');
						parts.forEach(part => {
							void (part as HTMLElement).offsetHeight;
						});
					}
				}, 150);
			}
	...
	private updateDynamicCSSRules(themeData: IColorTheme) {
		...
		// Critical fix: For light themes, force browser repaint by triggering resize
		// This ensures foreground colors (text) are visible on light backgrounds
		if (themeData.type === ColorScheme.LIGHT) {
			// Use multiple delayed attempts to force repaint
			[50, 150, 300].forEach(delay => {
				setTimeout(() => {
					...
					if (workbench) {
						...
						workbench.style.display = originalDisplay;
						void body.offsetHeight; // Force another reflow

						const resizeEvent = mainWindow.document.createEvent('UIEvents');
						resizeEvent.initEvent('resize', true, false);
						mainWindow.dispatchEvent(resizeEvent);
					}
				}, delay);
			});
		}
	...
	private applyTheme(newTheme: ColorThemeData, settingsTarget: ThemeSettingTarget, silent = false): Promise<IWorkbenchColorTheme | null> {
		...
		// CRITICAL FIX: For light themes in silent mode, schedule event firing
		// This prevents UI from freezing when components miss the initial event
		if (silent && newTheme.type === ColorScheme.LIGHT) {
			setTimeout(() => {
				this.onColorThemeChange.fire(this.currentColorTheme);

				// Additional fix: Ensure no elements are blocking interaction
				setTimeout(() => {
					const workbench = mainWindow.document.querySelector('.monaco-workbench') as HTMLElement;
					if (workbench) {
						workbench.style.pointerEvents = '';
					}
				}, 100);
			}, 200);
