...
import { IChatThreadService } from '../../senweaver/browser/chatThreadService.js';
import { SENWEAVER_VIEW_CONTAINER_ID } from '../../senweaver/browser/sidebarPane.js';
...
export class TerminalInstance extends Disposable implements ITerminalInstance {
	...
	private _open(): void {
		...
		// Senweaver: Add Send to Chat floating button on selection
		this._initSendToChatButton(xterm.raw.element!);
	...
	// Senweaver: Initialize Send to Chat floating button
	private _sendToChatButton: HTMLElement | undefined;
	private _savedSelectionText: string | undefined;

	private _initSendToChatButton(element: HTMLElement): void {
		// Create floating button
		const button = document.createElement('button');
		button.className = 'terminal-send-to-chat-button';
		...
		this._register(dom.addDisposableListener(button, 'click', async (e) => {
			e.stopPropagation();
			e.preventDefault();
			await this._sendSelectionToChat();
			this._hideSendToChatButton();
		}));

		// Handle mouseup to show button when selection exists
		this._register(dom.addDisposableListener(element, 'mouseup', (e: MouseEvent) => {
			// Small delay to let selection complete
			setTimeout(() => {
				if (this.hasSelection() && this.selection) {
					// Save selection text before showing button (it may be lost when button is clicked)
					this._savedSelectionText = this.selection;
					this._showSendToChatButton(e.clientX, e.clientY, element);
				} else {
					this._hideSendToChatButton();
				}
			}, 10);
		...
		// Hide button when clicking elsewhere or when selection changes
		this._register(dom.addDisposableListener(element, 'mousedown', (e: MouseEvent) => {
	...
	private _showSendToChatButton(clientX: number, clientY: number, container: HTMLElement): void {
		if (!this._sendToChatButton) return;

		const rect = container.getBoundingClientRect();
		const x = clientX - rect.left;
		const y = clientY - rect.top;

		this._sendToChatButton.style.left = `${x}px`;
		this._sendToChatButton.style.top = `${y + 10}px`;
		this._sendToChatButton.style.display = 'block';
	}

	private _hideSendToChatButton(): void {
		if (this._sendToChatButton) {
			this._sendToChatButton.style.display = 'none';
		}
	}

	private async _sendSelectionToChat(): Promise<void> {
		// Use saved selection text (current selection may be lost due to focus change)
		const selectedText = this._savedSelectionText || this.selection;
		if (!selectedText) return;

		// Clear saved selection
		this._savedSelectionText = undefined;

		// Open senweaver sidebar and wait for it to be ready
		await this._viewsService.openViewContainer(SENWEAVER_VIEW_CONTAINER_ID);

		const terminalName = this._title || 'Terminal';

		// Add terminal selection to chat as a staging selection
		this._chatThreadService.addNewStagingSelection({
			type: 'Terminal',
			terminalName: terminalName,
			terminalContent: selectedText,
		});

		// Small delay to ensure React component is mounted
		await new Promise(resolve => setTimeout(resolve, 100));

		// Insert terminal mention text at cursor position in chat input
		await this._chatThreadService.insertTextInCurrentChat(`@terminal:${terminalName} `);
