...
export function registerTerminalActions() {
	...
	// Senweaver: Register Send to Chat action
	registerSendToChatAction();
}
...
// Senweaver: Send terminal selection to chat
export function registerSendToChatAction(): IDisposable {
	return registerActiveXtermAction({
		id: TerminalCommandId.SendToChat,
		title: localize2('workbench.action.terminal.sendToChat', 'Send to Chat'),
		precondition: ContextKeyExpr.and(
			ContextKeyExpr.or(TerminalContextKeys.processSupported, TerminalContextKeys.terminalHasBeenCreated),
			TerminalContextKeys.textSelected
		),
		run: async (_xterm, accessor, instance) => {
			const chatThreadService = accessor.get(IChatThreadService);
			const viewsService = accessor.get(IViewsService);

			// Get selected text from terminal instance
			const selectedText = 'selection' in instance ? instance.selection : undefined;
			if (!selectedText) {
				return;
			}

			// Get terminal name
			const terminalName = 'title' in instance ? instance.title : 'Terminal';

			// Open senweaver sidebar
			...
			await chatThreadService.insertTextInCurrentChat(`@terminal:${terminalName} `);
