...
import { FeatureName, ModelSelection, ModelSelectionOptions, OverridesOfModel, ProviderName } from './senweaverSettingsTypes.js';


// Auto Mode 配置：当开启 Auto Mode 时，实际使用的模型
// 这个配置不展示给用户，只在内部使用
export const AUTO_MODE_MODEL: ModelSelection = {
...
export const defaultProviderSettings = {
	...
	ownProvider: {
		// 自有提供商统一配置（线上配置，所有模型共享相同的 baseURL 和 apiKey）
		// baseURL 在 OWN_PROVIDER_BASE_URL 常量中定义
		// apiKey 通过线上配置获取，所有模型共享
		apiKey: '', // 线上配置，不在设置面板显示
	},
	lmStudio: {
		endpoint: '',
	},
	...
	microsoftAzure: { // microsoft Azure Foundry
		...
		apiKey: '',
		azureApiVersion: '2025-05-01-preview',
	},
	awsBedrock: {
		apiKey: '',
...
export const defaultModelsOfProvider = {
	...
	aliBailian: [ // 阿里百炼模型服务 https://help.aliyun.com/zh/model-studio/
		//'qwen3-max-preview',
		//'qwen3-coder-plus',
		//'qwen-plus',
	],
	zAi: [ // 智谱AI https://open.bigmodel.cn/dev/api
		//'glm-4.6',
		//'glm-4-plus',
	],
	moonshotAi: [ // Moonshot AI https://platform.moonshot.cn/docs

	],
	ownProvider: [ // 自有提供商（线上配置，不在设置面板显示）
		'deepseek-chat',
		'z-ai/glm4.7',
...
// 模型功能标签类型
export type ModelFeatureTag = 'code' | 'plan' | 'new' | 'your-api-key';

export type SenweaverStaticModelInfo = { // not stateful
	// uses the information below to know how to handle each model.
	// for some examples, see openAIModelOptions and anthropicModelOptions (below).

	// 显示名称：用于在 UI 中显示的模型别名，默认使用模型原始名称
	displayName?: string;
	...
	supportsVision: boolean; // whether the model supports multimodal image input
	isFree?: boolean;
	...
	// 模型功能标签：code（代码生成）、plan（规划）、new（最新支持）
	featureTags?: ModelFeatureTag[];
...
type SenweaverStaticProviderInfo = { // doesn't change (not stateful)
	...
	modelOptions: { [key: string]: SenweaverStaticModelInfo };
	modelOptionsFallback: (modelName: string, fallbackKnownValues?: Partial<SenweaverStaticModelInfo>) => (SenweaverStaticModelInfo & { modelName: string, recognizedModelName: string }) | null;
}
...
const defaultModelOptions = {
	...
	reasoningCapabilities: false,
	isFree: false, // 默认为付费
} as const satisfies SenweaverStaticModelInfo
...
const openSourceModelOptions_assumingOAICompat = {
	'deepseekR1': {
		supportsFIM: false,  // R1 是推理模型，不支持 FIM
		supportsVision: false,
		supportsSystemMessage: false,
...
} as const satisfies { [s: string]: Partial<SenweaverStaticModelInfo> }




// keep modelName, but use the fallback's defaults
const extensiveModelOptionsFallback: SenweaverStaticProviderInfo['modelOptionsFallback'] = (modelName, fallbackKnownValues) => {

	const lower = modelName.toLowerCase()

	const toFallback = <T extends { [s: string]: Omit<SenweaverStaticModelInfo, 'cost' | 'downloadable'> },>(obj: T, recognizedModelName: string & keyof T)
		: SenweaverStaticModelInfo & { modelName: string, recognizedModelName: string } => {
...
} as const satisfies { [s: string]: SenweaverStaticModelInfo }

const anthropicSettings: SenweaverStaticProviderInfo = {
...
const openAISettings: SenweaverStaticProviderInfo = {
...
const xAISettings: SenweaverStaticProviderInfo = {
...
const geminiSettings: SenweaverStaticProviderInfo = {
...
const deepseekModelOptions = {
	'deepseek-chat': {
		...
		downloadable: false,
		supportsVision: false,
		supportsFIM: true,  // 通过 beta API 支持 FIM，最大补全长度 4K
	},
	'deepseek-reasoner': {
		...openSourceModelOptions_assumingOAICompat.deepseekR1,
...
const aliBailianModelOptions = { // https://help.aliyun.com/zh/model-studio/
	'qwen3-max-preview': {
...
const aliBailianSettings: SenweaverStaticProviderInfo = {
	modelOptions: aliBailianModelOptions,
	modelOptionsFallback: (modelName) => { return null },
	providerReasoningIOSettings: {
		// 阿里百炼使用 OpenAI 兼容格式
		input: { includeInPayload: openAICompatIncludeInPayloadReasoning },
...
const zAiModelOptions = { // https://open.bigmodel.cn/dev/api
	'glm-4.6': {
		contextWindow: 200_000,
		reservedOutputTokenSpace: 128_000,
		cost: { input: 0.10, output: 0.20 }, // 免费模型
		supportsFIM: false,
		supportsVision: false,
		downloadable: false,
		supportsSystemMessage: 'system-role',
...
const zAiSettings: SenweaverStaticProviderInfo = {
...
const moonshotAiModelOptions = { // https://platform.moonshot.cn/docs
	'kimi-k2-thinking': {
...
const moonshotAiSettings: SenweaverStaticProviderInfo = {
	modelOptions: moonshotAiModelOptions,
	modelOptionsFallback: (modelName) => { return null },
	providerReasoningIOSettings: {
		// Moonshot AI 使用 OpenAI 兼容格式
		input: { includeInPayload: openAICompatIncludeInPayloadReasoning },
		output: { nameOfFieldInDelta: 'reasoning_content' },
	},
}
...
// 自有提供商统一配置（线上配置，所有模型共享）
export const OWN_PROVIDER_BASE_URL = 'https://api.newpoc.com/v1';

const ownProviderModelOptions = { // 线上配置，不在设置面板显示
	'deepseek-chat': {
		displayName: 'deepseek-v3.2',
		contextWindow: 128_000,
		reservedOutputTokenSpace: 8_000,
		cost: { input: 0.01, output: 0.10 },
		downloadable: false,
		supportsFIM: true,
		supportsVision: false,
		supportsSystemMessage: 'system-role',
		reasoningCapabilities: false,
		featureTags: ['code', 'plan'],
	},
	'gpt-5.2-codex': {
		contextWindow: 128_000,
		reservedOutputTokenSpace: null,
...
const ownProviderSettings: SenweaverStaticProviderInfo = {
	modelOptions: ownProviderModelOptions,
	modelOptionsFallback: (modelName) => { return null },
	providerReasoningIOSettings: {
		// 使用 OpenAI 兼容格式
		input: { includeInPayload: openAICompatIncludeInPayloadReasoning },
...
const deepseekSettings: SenweaverStaticProviderInfo = {
...
const mistralSettings: SenweaverStaticProviderInfo = {
...
const groqSettings: SenweaverStaticProviderInfo = {
...
} as const satisfies Record<string, SenweaverStaticModelInfo>
const googleVertexSettings: SenweaverStaticProviderInfo = {
...
const microsoftAzureSettings: SenweaverStaticProviderInfo = {
...
const awsBedrockSettings: SenweaverStaticProviderInfo = {
...
export const ollamaRecommendedModels = ['qwen2.5-coder:7b', 'llama3.1', 'qwq', 'deepseek-r1', 'devstral:latest'] as const satisfies (keyof typeof ollamaModelOptions)[]


const vLLMSettings: SenweaverStaticProviderInfo = {
	modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName, { downloadable: { sizeGb: 'not-known' }, isFree: true }),
	modelOptions: {},
	providerReasoningIOSettings: {
...
const lmStudioSettings: SenweaverStaticProviderInfo = {
	modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName, { downloadable: { sizeGb: 'not-known' }, contextWindow: 4_096, isFree: true }),
	modelOptions: {},
	providerReasoningIOSettings: {
		input: { includeInPayload: openAICompatIncludeInPayloadReasoning },
...
const ollamaSettings: SenweaverStaticProviderInfo = {
	modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName, { downloadable: { sizeGb: 'not-known' }, isFree: true }),
...
const openaiCompatible: SenweaverStaticProviderInfo = {
	modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName, { isFree: true }),
	modelOptions: {},
	providerReasoningIOSettings: {
...
const liteLLMSettings: SenweaverStaticProviderInfo = { // https://docs.litellm.ai/docs/reasoning_content
	modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName, { downloadable: { sizeGb: 'not-known' }, isFree: true }),
	modelOptions: {},
	providerReasoningIOSettings: {
		input: { includeInPayload: openAICompatIncludeInPayloadReasoning },
...
const openRouterModelOptions_assumingOpenAICompat = {
	...
	'anthropic/claude-sonnet-4.5': {
		...
		cost: { input: 3.00, output: 15.00 },
		downloadable: false,
		supportsFIM: false,  // Claude 不支持 FIM
		supportsVision: true,
		supportsSystemMessage: 'system-role',
		reasoningCapabilities: false,
	},
	...
	'mistralai/codestral-2508': {
		...
		downloadable: false,
		supportsFIM: false,  // OpenRouter 不支持 FIM API 格式
		reasoningCapabilities: false,
	},
	'mistralai/devstral-small:free': {
...
const openRouterSettings: SenweaverStaticProviderInfo = {
...
const modelSettingsOfProvider: { [providerName in ProviderName]: SenweaverStaticProviderInfo } = {
	...
	groq: groqSettings,
	aliBailian: aliBailianSettings,
	zAi: zAiSettings,
	moonshotAi: moonshotAiSettings,
	ownProvider: ownProviderSettings,
