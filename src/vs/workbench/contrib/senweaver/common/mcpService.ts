...
import { MCPUserStateOfName } from './senweaverSettingsTypes.js';
...
class MCPService extends Disposable implements IMCPService {
	...
	constructor(
		...
		super();
		this.channel = this.mainProcessService.getChannel('senweaver-channel-mcp')
	...
	private async _initialize() {
		try {
			await this.SenweaverSettingsService.waitForInitState;
	...
	private async _refreshMCPServers(): Promise<void> {
		...
		await this.SenweaverSettingsService.addMCPUserStateOfNames(addedUserStateOfName);

		// delete isOn for any servers that no longer show up in the config
		await this.SenweaverSettingsService.removeMCPUserStateOfNames(removedServerNames);
		...
		this.channel.call('refreshMCPServers', {
			...
			removedServerNames,
			updatedServerNames,
			userStateOfName: this.SenweaverSettingsService.state.mcpUserStateOfName,
		})
	}
	...
	// toggle MCP server and update isOn in senweaver settings
	public async toggleServerIsOn(serverName: string, isOn: boolean): Promise<void> {
		this._setMCPServerState(serverName, { status: 'loading', tools: [] })

		await this.SenweaverSettingsService.setMCPServerState(serverName, { isOn });
