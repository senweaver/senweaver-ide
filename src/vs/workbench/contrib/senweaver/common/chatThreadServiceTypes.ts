...
import { SenweaverFileSnapshot } from './editCodeServiceTypes.js';
...
export type CheckpointEntry = {
	...
	senweaverFileSnapshotOfURI: { [fsPath: string]: SenweaverFileSnapshot | undefined };
...
// Image attachment upload status
export type ImageUploadStatus = 'pending' | 'uploading' | 'uploaded' | 'error';

// Image attachment type for multimodal support
export type ImageAttachment = {
	id: string;
	name: string;
	mimeType: string;
	size: number;
	base64Data?: string; // base64 encoded image data (仅用于上传前的本地预览，上传后清除)
	width?: number;
	height?: number;
	thumbnailBase64?: string; // optional thumbnail for UI (小图用于消息列表显示)
	// 图片上传相关字段
	uploadStatus?: ImageUploadStatus; // 上传状态
	uploadedUrl?: string; // 上传成功后的图片URL（主要存储字段）
...
export type ChatMessage =
	| {
		...
		images?: ImageAttachment[]; // multimodal image attachments
		state: {
