...
import { SENWEAVER_SETTINGS_STORAGE_KEY } from './storageKeys.js';
import { defaultSettingsOfProvider, FeatureName, ProviderName, ModelSelectionOfFeature, SettingsOfProvider, SettingName, providerNames, ModelSelection, modelSelectionsEqual, featureNames, SenweaverStatefulModelInfo, GlobalSettings, GlobalSettingName, defaultGlobalSettings, ModelSelectionOptions, OptionsOfModelSelection, ChatMode, OverridesOfModel, defaultOverridesOfModel, MCPUserStateOfName as MCPUserStateOfName, MCPUserState } from './senweaverSettingsTypes.js';
...
export type SenweaverSettingsState = {
...
// type RealSenweaverSettings = Exclude<keyof SenweaverSettingsState, '_modelOptions'>
// type EventProp<T extends RealSenweaverSettings = RealSenweaverSettings> = T extends 'globalSettings' ? [T, keyof SenweaverSettingsState[T]] : T | 'all'


export interface ISenweaverSettingsService {
	readonly _serviceBrand: undefined;
	readonly state: SenweaverSettingsState; // in order to play nicely with react, you should immutably change state
	...
	dangerousSetState(newState: SenweaverSettingsState): Promise<void>;
...
const _modelsWithSwappedInNewModels = (options: { existingModels: SenweaverStatefulModelInfo[], models: string[], type: 'autodetected' | 'default' }) => {
	const { existingModels, models, type } = options

	const existingModelsMap: Record<string, SenweaverStatefulModelInfo> = {}
...
const _stateWithMergedDefaultModels = (state: SenweaverSettingsState): SenweaverSettingsState => {
...
const _validatedModelState = (state: Omit<SenweaverSettingsState, '_modelOptions'>): SenweaverSettingsState => {
	...
	for (const providerName of providerNames) {
		...
		// 检查提供商设置是否填写完整（所有提供商统一处理，包括 ownProvider）
	...
	} satisfies SenweaverSettingsState
...
const defaultState = () => {
	const d: SenweaverSettingsState = {
...
export const ISenweaverSettingsService = createDecorator<ISenweaverSettingsService>('SenweaverSettingsService');
class SenweaverSettingsService extends Disposable implements ISenweaverSettingsService {
	_serviceBrand: undefined;

	private readonly _onDidChangeState = new Emitter<void>();
	readonly onDidChangeState: Event<void> = this._onDidChangeState.event; // this is primarily for use in react, so react can listen + update on state changes

	state: SenweaverSettingsState;
	...
	dangerousSetState = async (newState: SenweaverSettingsState) => {
	...
	async readAndInitializeState() {
		let readS: SenweaverSettingsState
		try {
	...
	private async _readState(): Promise<SenweaverSettingsState> {
		const encryptedState = this._storageService.get(SENWEAVER_SETTINGS_STORAGE_KEY, StorageScope.APPLICATION)
	...
	private async _storeState() {
		...
		this._storageService.store(SENWEAVER_SETTINGS_STORAGE_KEY, encryptedState, StorageScope.APPLICATION, StorageTarget.USER);
	}
	...
	setGlobalSetting: SetGlobalSettingFn = async (settingName, newVal) => {
		const newState: SenweaverSettingsState = {
			...this.state,
			globalSettings: {
	...
	toggleModelHidden(providerName: ProviderName, modelName: string) {
		...
		const newModels: SenweaverStatefulModelInfo[] = [
...
registerSingleton(ISenweaverSettingsService, SenweaverSettingsService, InstantiationType.Eager);
