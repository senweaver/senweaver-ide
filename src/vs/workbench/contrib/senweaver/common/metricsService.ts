...
export class MetricsService implements IMetricsService {
	...
	constructor(
		...
		this.metricsService = ProxyChannel.toService<IMetricsService>(mainProcessService.getChannel('senweaver-channel-metrics'));
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'senweaverDebugInfo',
			f1: true,
			title: localize2('senweaverMetricsDebug', 'SenWeaver: Log Debug Info'),
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		notifService.info(`SenWeaver Debug info:\n${JSON.stringify(debugProperties, null, 2)}`)
	}
})
