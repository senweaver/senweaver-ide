...
import { ChatMode, displayInfoOfProviderName, ModelSelectionOptions, OverridesOfModel, ProviderName, SettingsOfProvider } from '../../common/senweaverSettingsTypes.js';
import { getSendableReasoningInfo, getModelCapabilities, getProviderCapabilities, defaultProviderSettings, getReservedOutputTokenSpace, OWN_PROVIDER_BASE_URL } from '../../common/modelCapabilities.js';
...
const SENWEAVER_UNIFIED_UA = (() => {
	// 统一 UA：尽量模拟 Electron/Chrome（避免 Cloudflare “Manage AI bots” 因 OpenAI/JS UA 误杀）
	// 同时带上产品标识，便于服务端日志识别。
	const platform = process.platform === 'win32' ? 'Windows NT 10.0; Win64; x64'
		: process.platform === 'darwin' ? 'Macintosh; Intel Mac OS X 10_15_7'
			: 'X11; Linux x86_64';
	return `Mozilla/5.0 (${platform}) AppleWebKit/537.36 (KHTML, like Gecko) SenWeaver/1.0 Chrome/121.0.0.0 Safari/537.36`;
})();

function withUnifiedUA(headers?: Record<string, string | null | undefined>): Record<string, string> {
	...
	// 强制覆盖 UA，确保统一
	merged['User-Agent'] = SENWEAVER_UNIFIED_UA;
	return merged;
}

const newOpenAICompatibleSDK = async ({ settingsOfProvider, providerName, includeInPayload, isFIM }: { settingsOfProvider: SettingsOfProvider, providerName: ProviderName, includeInPayload?: { [s: string]: any }, isFIM?: boolean }) => {
	...
	if (providerName === 'openAI') {
		...
		return new OpenAI({ apiKey: thisConfig.apiKey, defaultHeaders: withUnifiedUA(), ...commonPayloadOpts })
	}
	else if (providerName === 'ollama') {
		const thisConfig = settingsOfProvider[providerName]
		return new OpenAI({ baseURL: `${thisConfig.endpoint}/v1`, apiKey: 'noop', defaultHeaders: withUnifiedUA(), ...commonPayloadOpts })
	}
		...
		return new OpenAI({
			...
			defaultHeaders: withUnifiedUA({
				'HTTP-Referer': 'https://senweaver.com', // Optional, for including your app on openrouter.ai rankings.
				'X-Title': 'SenWeaver', // Optional. Shows in rankings on openrouter.ai.
			}),
			...commonPayloadOpts,
		})
	}
		...
		return new OpenAI({ baseURL: baseURL, apiKey: apiKey, defaultHeaders: withUnifiedUA(), ...commonPayloadOpts })
	}
		...
		return new OpenAI({ baseURL, apiKey, defaultHeaders: withUnifiedUA(), ...commonPayloadOpts })
	}


	else if (providerName === 'deepseek') {
		const thisConfig = settingsOfProvider[providerName]
		// FIM 需要使用 beta API，聊天使用标准 API
		const baseURL = isFIM ? 'https://api.deepseek.com/beta' : 'https://api.deepseek.com/v1'
		return new OpenAI({ baseURL, apiKey: thisConfig.apiKey, defaultHeaders: withUnifiedUA(), ...commonPayloadOpts })
	}
	else if (providerName === 'openAICompatible') {
		const thisConfig = settingsOfProvider[providerName]
		const headers = parseHeadersJSON(thisConfig.headersJSON)
		return new OpenAI({ baseURL: thisConfig.endpoint, apiKey: thisConfig.apiKey, defaultHeaders: withUnifiedUA(headers), ...commonPayloadOpts })
	}
	else if (providerName === 'groq') {
		const thisConfig = settingsOfProvider[providerName]
		return new OpenAI({ baseURL: 'https://api.groq.com/openai/v1', apiKey: thisConfig.apiKey, defaultHeaders: withUnifiedUA(), ...commonPayloadOpts })
	}
	else if (providerName === 'xAI') {
		const thisConfig = settingsOfProvider[providerName]
		return new OpenAI({ baseURL: 'https://api.x.ai/v1', apiKey: thisConfig.apiKey, defaultHeaders: withUnifiedUA(), ...commonPayloadOpts })
	}
	else if (providerName === 'mistral') {
		const thisConfig = settingsOfProvider[providerName]
		return new OpenAI({ baseURL: 'https://api.mistral.ai/v1', apiKey: thisConfig.apiKey, defaultHeaders: withUnifiedUA(), ...commonPayloadOpts })
	}
	else if (providerName === 'aliBailian') {
		const thisConfig = settingsOfProvider[providerName]
		return new OpenAI({ baseURL: 'https://dashscope.aliyuncs.com/compatible-mode/v1', apiKey: thisConfig.apiKey, defaultHeaders: withUnifiedUA(), ...commonPayloadOpts })
	}
	else if (providerName === 'zAi') {
		const thisConfig = settingsOfProvider[providerName]
		return new OpenAI({ baseURL: 'https://open.bigmodel.cn/api/coding/paas/v4', apiKey: thisConfig.apiKey, defaultHeaders: withUnifiedUA(), ...commonPayloadOpts })
	}
	else if (providerName === 'moonshotAi') {
		const thisConfig = settingsOfProvider[providerName]
		return new OpenAI({ baseURL: 'https://api.moonshot.cn/v1', apiKey: thisConfig.apiKey, defaultHeaders: withUnifiedUA(), ...commonPayloadOpts })
	}
	else if (providerName === 'ownProvider') {
		// 自有提供商：所有模型共享相同的 baseURL 和 apiKey（线上配置）
		const thisConfig = settingsOfProvider[providerName]
		const baseURL = OWN_PROVIDER_BASE_URL
		const apiKey = thisConfig.apiKey
		return new OpenAI({ baseURL, apiKey, defaultHeaders: withUnifiedUA(), ...commonPayloadOpts })
	}

	else throw new Error(`SenWeaver providerName was invalid: ${providerName}.`)
}
...
const _sendOpenAICompatibleFIM = async ({ messages: { prefix, suffix, stopTokens }, onFinalMessage, onError, settingsOfProvider, modelName: modelName_, _setAborter, providerName, overridesOfModel }: SendFIMParams_Internal) => {
	...
	// OpenRouter 不支持标准的 FIM API，需要特殊处理
	if (providerName === 'openRouter') {
		onError({ message: `OpenRouter does not support FIM API format. Please use Ollama or DeepSeek for code completion.`, fullError: null })
		return
	}

	const openai = await newOpenAICompatibleSDK({ providerName, settingsOfProvider, includeInPayload: additionalOpenAIPayload, isFIM: true })

	// 构建请求参数，只在 stopTokens 非空时才传递 stop 参数
	const requestParams: any = {
		model: modelName,
		prompt: prefix,
		suffix: suffix,
		max_tokens: 4096,  // DeepSeek FIM 最大补全长度 4K
	}
	// 只有当 stopTokens 有内容时才添加 stop 参数
	if (stopTokens && stopTokens.length > 0) {
		requestParams.stop = stopTokens
	}
	...
	openai.completions
		...
		.catch(error => {
			...
			if (error instanceof OpenAI.APIError && error.status === 401) {
				...
				console.error('FIM Error Details:', { providerName, modelName, errorMessage, error })
				onError({ message: `FIM Error (${providerName}/${modelName}): ${errorMessage}`, fullError: error });
			}
		})
}
...
const openAITools = (chatMode: ChatMode | null, mcpTools: InternalToolInfo[] | undefined, supportsVision?: boolean) => {
...
const _sendOpenAICompatibleChat = async ({ messages, onText, onFinalMessage, onError, settingsOfProvider, modelSelectionOptions, modelName: modelName_, _setAborter, providerName, chatMode, separateSystemMessage, overridesOfModel, mcpTools }: SendChatParams_Internal) => {
	const modelCapabilities = getModelCapabilities(providerName, modelName_, overridesOfModel)
	const {
		modelName,
		specialToolFormat,
	...
	// tools - 当模型支持视觉理解时，不提供 analyze_image 工具
	...
	openai.chat.completions
		...
		.then(async response => {
			...
			if (!fullTextSoFar && !fullReasoningSoFar && !toolName) {
				onError({ message: 'SenWeaver: Response from model was empty. This may happen if the conversation is too long - try starting a new chat or the model may be temporarily unavailable.', fullError: null })
			}
			else {
...
const anthropicTools = (chatMode: ChatMode | null, mcpTools: InternalToolInfo[] | undefined, supportsVision?: boolean) => {
	const allowedTools = availableTools(chatMode, mcpTools, supportsVision)
	if (!allowedTools || Object.keys(allowedTools).length === 0) return null
...
const newOllamaSDK = ({ endpoint }: { endpoint: string }) => {
	...
	if (!endpoint) throw new Error(`Ollama Endpoint was empty (please enter ${defaultProviderSettings.ollama.endpoint} in SenWeaver if you want the default url).`)
...
const sendOllamaFIM = ({ messages, onFinalMessage, onError, settingsOfProvider, modelName, _setAborter }: SendFIMParams_Internal) => {
	...
	ollama.generate({
		...
		options: {
			stop: messages.stopTokens,
			num_predict: 1000, // 增加到 1000 tokens
			// repeat_penalty: 1,
		},
		raw: true,
...
const geminiTools = (chatMode: ChatMode | null, mcpTools: InternalToolInfo[] | undefined, supportsVision?: boolean): GeminiTool[] | null => {
	const allowedTools = availableTools(chatMode, mcpTools, supportsVision)
	if (!allowedTools || Object.keys(allowedTools).length === 0) return null
...
export const sendLLMMessageToProviderImplementation = {
	...
	ownProvider: {
		sendChat: (params) => _sendOpenAICompatibleChat(params),
		sendFIM: (params) => _sendOpenAICompatibleFIM(params),  // 支持 OpenAI 兼容的 FIM
		list: null,
	},
	ollama: {
		sendChat: (params) => _sendOpenAICompatibleChat(params),
	...
	deepseek: {
		sendChat: (params) => _sendOpenAICompatibleChat(params),
		sendFIM: (params) => _sendOpenAICompatibleFIM(params),  // 使用 beta API 支持 FIM
		list: null,
	},
	groq: {
		sendChat: (params) => _sendOpenAICompatibleChat(params),
