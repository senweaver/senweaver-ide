...
import { ChatMode } from '../../common/senweaverSettingsTypes.js'
...
// Parameter aliases for AI model compatibility - maps alternative names to standard names
// NOTE: Be careful with aliases that might conflict with AI-generated metadata tags
// e.g., 'folder', 'type', 'file' are NOT included because AI often outputs <folder>true</folder> as metadata
const PARAM_ALIASES: { [alias: string]: string } = {
	// URI/path aliases -> uri (only unambiguous ones)
	'path': 'uri',
	'file_path': 'uri',
	'filepath': 'uri',
	// 'file': 'uri', // Removed - AI might output <file>true</file>
	// 'folder': 'uri', // Removed - AI might output <folder>true</folder>
	'directory': 'uri',
	'dir': 'uri',
	'target': 'uri',
	'location': 'uri',
	// Content aliases -> new_content (only unambiguous ones)
	// 'content': 'new_content', // Removed - too generic, might conflict
	'file_content': 'new_content',
	// 'text': 'new_content', // Removed - AI might use for other purposes
	// 'code': 'new_content', // Removed - AI might use for other purposes
	// 'data': 'new_content', // Removed - too generic
	// 'body': 'new_content', // Removed - too generic
	// Query aliases -> query
	'search': 'query',
	'search_query': 'query',
	'keyword': 'query',
	'keywords': 'query',
	'term': 'query',
	// 'pattern': 'query', // Removed - might conflict with regex pattern
...
const parseXMLPrefixToToolCall = <T extends ToolName,>(toolName: T, toolId: string, str: string, toolOfToolName: ToolOfToolName): RawToolCallObj => {
	...
	// Get allowed params and add their aliases
	const toolParams = toolOfToolName[toolName]?.params ?? {}
	const allowedParams = Object.keys(toolParams) as ToolParamName<T>[]

	// Build a map from all possible param names (including aliases) to standard param names
	const paramNameToStandard: { [key: string]: ToolParamName<T> } = {}
	for (const param of allowedParams) {
		paramNameToStandard[param] = param
	}
	// Add aliases that map to allowed params
	for (const [alias, standard] of Object.entries(PARAM_ALIASES)) {
		if (allowedParams.includes(standard as ToolParamName<T>)) {
			paramNameToStandard[alias] = standard as ToolParamName<T>
		}
	}
	const allPossibleParams = Object.keys(paramNameToStandard)
	...
	while (true) {
		...
		// find the param name opening tag (check all possible params including aliases)
		let matchedOpenParam: null | ToolParamName<T> = null
		let matchedTagName: string | null = null
		for (const possibleName of allPossibleParams) {
			const removed = pm.removeFromStartUntilFullMatch(`<${possibleName}>`, true)
			if (removed) {
				// Map alias to standard param name
				matchedOpenParam = paramNameToStandard[possibleName]
				matchedTagName = possibleName
				break
			}
		}
		...
		// find the param name closing tag (use the same tag name that was opened)
		let matchedCloseParam: boolean = false
		let paramContents = ''
		// First try to match the same tag that was opened
		const tagsToTry = matchedTagName ? [matchedTagName, ...allPossibleParams.filter(p => p !== matchedTagName)] : allPossibleParams
		for (const possibleName of tagsToTry) {
			const i = pm.i
			const closeTag = `</${possibleName}>`
