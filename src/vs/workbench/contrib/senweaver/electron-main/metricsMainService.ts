...
export class MetricsMainService extends Disposable implements IMetricsService {
	...
	// helper - looks like this is stored in a .vscdb file in ~/Library/Application Support/SenWeaver
	...
	private get oldId() {
		...
		const newKey = 'senweaver.app.oldMachineId'
		...
		const oldValue = this._appStorage.get('senweaver.machineId', StorageScope.APPLICATION) ?? 'NULL' // the old way of getting the key
		...
		// private get oldId() {
		// 	return this._memoStorage('senweaver.app.oldMachineId', StorageTarget.MACHINE, 'NULL')
		// }
	}
	...
	private get distinctId() {
		...
		return this._memoStorage('senweaver.app.machineId', StorageTarget.MACHINE, setValIfNotExist)
	}

	// just to see if there are ever multiple machineIDs per userID (instead of this, we should just track by the user's email)
	private get userId() {
		return this._memoStorage('senweaver.app.userMachineId', StorageTarget.USER)
	}
	...
	async initialize() {
		...
		const { commit, version, senweaverVersion, release, quality } = this._productService
		...
		this._initProperties = {
			...
			senweaverVersion: senweaverVersion,
			release,
			os,
			quality,
		...
		console.log('User is opted out of basic SenWeaver metrics?', didOptOut)
		if (didOptOut) {
			this.client.optOut()
		}
		else {
		...
		console.log('SenWeaver posthog metrics info:', JSON.stringify(identifyMessage, null, 2))
	}
