...
import { displayInfoOfProviderName, ProviderName, providerNames, localProviderNames, featureNames, FeatureName, isFeatureNameDisabled } from '../../../../common/senweaverSettingsTypes.js';
import { ChatMarkdownRender } from '../markdown/ChatMarkdownRender.js';
import { OllamaSetupInstructions, OneClickSwitchButton, SettingsForProvider, ModelDump } from '../senweaver-settings-tsx/Settings.js';
...
export const SenweaverOnboarding = () => {

	const SenweaverSettingsState = useSettingsState()
	const isOnboardingComplete = SenweaverSettingsState.globalSettings.isOnboardingComplete || OVERRIDE_VALUE
	...
	return (
		<div className={`@@senweaver-scope ${isDark ? 'dark' : ''}`}>
			<div
				className={`
					bg-senweaver-bg-3 fixed top-0 right-0 bottom-0 left-0 width-full z-[99999]
...
const SenweaverIcon = () => {
	...
	return <div ref={divRef} className='@@senweaver-senweaver-icon' />
}
...
const tabNames = ['免费', '付费'] as const;

type TabName = typeof tabNames[number] | '其他';
...
const providerNamesOfTab: Record<TabName, ProviderName[]> = {
	免费: ['gemini', 'openRouter'],
	//本地: localProviderNames,
	付费: providerNames.filter(pn => !(['gemini', 'openRouter', ...localProviderNames, ...cloudProviders] as string[]).includes(pn)) as ProviderName[],
	'其他': cloudProviders,
};

const descriptionOfTab: Record<TabName, string> = {
	免费: `提供100%免费等级的提供商。添加任意数量！`,
	付费: `直接连接任何提供商（提供您自己的密钥）。`,
	//本地: `活动的本地提供商应自动显示。添加任意数量！`,
	'其他': `添加任意数量！如果需要自定义配置，请联系我们。`,
};
...
const AddProvidersPage = ({ pageIndex, setPageIndex }: { pageIndex: number, setPageIndex: (index: number) => void }) => {
	const [currentTab, setCurrentTab] = useState<TabName>('免费');
	...
	return (<div className="flex flex-col md:flex-row w-full h-[80vh] gap-6 max-w-[900px] mx-auto relative">
		{/* Left Column */}
		<div className="md:w-1/4 w-full flex flex-col gap-6 p-6 border-none border-senweaver-border-2 h-full overflow-y-auto">
			{/* Tab Selector */}
		...
		<div className="flex-1 flex flex-col items-center justify-start p-6 h-full overflow-y-auto">
			<div className="text-5xl mb-2 text-center w-full">添加提供商</div>

			<div className="w-full max-w-xl mt-4 mb-10">
				<div className="text-4xl font-light my-4 w-full">{currentTab}</div>
				<div className="text-sm opacity-80 text-senweaver-fg-3 my-4 w-full">{descriptionOfTab[currentTab]}</div>
			</div>
			...
			{providerNamesOfTab[currentTab].map((providerName) => (
				<div key={providerName} className="w-full max-w-xl mb-10">
					<div className="text-xl mb-2">
						...
						{providerName === 'gemini' && (
							<span
								data-tooltip-id="senweaver-tooltip-provider-info"
			...
			{(/* currentTab === '本地' ||  */currentTab === '其他') && (
				<div className="w-full max-w-xl mt-8 bg-senweaver-bg-2/50 rounded-lg p-6 border border-senweaver-border-4">
					<div className="flex items-center gap-2 mb-4">
						<div className="text-xl font-medium">Models</div>
					</div>

					{/* {currentTab === '本地' && (
						<div className="text-sm opacity-80 text-senweaver-fg-3 my-4 w-full">Local models should be detected automatically. You can add custom models below.</div>
					)} */}

					{/* {currentTab === '本地' && <ModelDump filteredProviders={localProviderNames} />} */}
					{currentTab === '其他' && <ModelDump filteredProviders={cloudProviders} />}
				</div>
			)}
			...
			<div className="flex flex-col items-end w-full mt-auto pt-8">
				...
				<div className="flex items-center gap-2">
					...
					<NextButton
						onClick={() => {
							// 如果是第一次使用（引导流程未完成），允许跳过验证
							const isFirstTimeUse = !settingsState.globalSettings.isOnboardingComplete;
							const isDisabled = isFirstTimeUse ? false : isFeatureNameDisabled('Chat', settingsState);
...
// 		title:
// 			div
// 				"How would you like to use Senweaver?"
// 		content:
...
const PreviousButton = ({ onClick, ...props }: { onClick: () => void } & React.ButtonHTMLAttributes<HTMLButtonElement>) => {
	return (
		<button
			onClick={onClick}
			className="px-6 py-2 rounded text-senweaver-fg-3 opacity-80 hover:brightness-115 duration-600 transition-all"
			{...props}
		>
			Back
		</button>
...
const OllamaDownloadOrRemoveModelButton = ({ modelName, isModelInstalled, sizeGb }: { modelName: string, isModelInstalled: boolean, sizeGb: number | false | 'not-known' }) => {
	...
	return <a
		...
		className="flex items-center justify-center text-senweaver-fg-2 hover:text-senweaver-fg-1"
	>
...
const SenweaverOnboardingContent = () => {
	...
	const didFillInProviderSettings = selectedProviderName && SenweaverSettingsState.settingsOfProvider[selectedProviderName]._didFillInProviderSettings
	const isApiKeyLongEnoughIfApiKeyExists = selectedProviderName && SenweaverSettingsState.settingsOfProvider[selectedProviderName].apiKey ? SenweaverSettingsState.settingsOfProvider[selectedProviderName].apiKey.length > 15 : true
	const isAtLeastOneModel = selectedProviderName && SenweaverSettingsState.settingsOfProvider[selectedProviderName].models.length >= 1
	...
	const lastPagePrevAndNextButtons = <div className="max-w-[600px] w-full mx-auto flex flex-col items-end">
		<div className="flex items-center gap-2">
			...
			<PrimaryActionButton
				onClick={() => {
					SenweaverSettingsService.setGlobalSetting('isOnboardingComplete', true);
					voidMetricsService.capture('Completed Onboarding', { selectedProviderName, wantToUseOption })
				}}
				ringSize={SenweaverSettingsState.globalSettings.isOnboardingComplete ? 'screen' : undefined}
			>Enter the SenWeaver</PrimaryActionButton>
		</div>
	</div>
	...
	const detailedDescOfWantToUseOption: { [wantToUseOption in WantToUseOption]: string } = {
		...
		private: "Private-hosted so your data never leaves your computer or network. [Email us](mailto:founders@SenweaverEditor.com) for help setting up at your company.",
	...
	useEffect(() => {
		if (!SenweaverSettingsState.globalSettings.isOnboardingComplete) {
			setPageIndex(0)
		}
	}, [setPageIndex, SenweaverSettingsState.globalSettings.isOnboardingComplete])
	...
	const contentOfIdx: { [pageIndex: number]: React.ReactNode } = {
		0: <OnboardingPageShell
			content={
				<div className='flex flex-col items-center gap-8'>
					<div className="text-5xl font-light text-center">Welcome to SenWeaver</div>

					{/* Slice of Senweaver image */}
					<div className='max-w-md w-full h-[30vh] mx-auto flex items-center justify-center'>
						{!isLinux && <SenweaverIcon />}
					</div>
		...
		2: <OnboardingPageShell
			...
			content={
				<div>
					<div className="text-5xl font-light text-center">主题和设置</div>

					<div className="mt-8 text-center flex flex-col items-center gap-4 w-full max-w-md mx-auto">
						<h4 className="text-senweaver-fg-3 mb-4">是否从现有编辑器转移设置？</h4>
