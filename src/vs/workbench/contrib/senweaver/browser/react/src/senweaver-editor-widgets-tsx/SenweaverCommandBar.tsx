...
import { SenweaverCommandBarProps } from '../../../senweaverCommandBarService.js';
import { Check, EllipsisVertical, Menu, MoveDown, MoveLeft, MoveRight, MoveUp, X } from 'lucide-react';
import {
	SENWEAVER_GOTO_NEXT_DIFF_ACTION_ID,
	SENWEAVER_GOTO_PREV_DIFF_ACTION_ID,
	SENWEAVER_GOTO_NEXT_URI_ACTION_ID,
	SENWEAVER_GOTO_PREV_URI_ACTION_ID,
	SENWEAVER_ACCEPT_FILE_ACTION_ID,
	SENWEAVER_REJECT_FILE_ACTION_ID,
	SENWEAVER_ACCEPT_ALL_DIFFS_ACTION_ID,
	SENWEAVER_REJECT_ALL_DIFFS_ACTION_ID
} from '../../../actionIDs.js';

export const SenweaverCommandBarMain = ({ uri, editor }: SenweaverCommandBarProps) => {
	const isDark = useIsDark()

	return <div
		className={`@@senweaver-scope ${isDark ? 'dark' : ''}`}
	>
		<SenweaverCommandBar uri={uri} editor={editor} />
	</div>
}
...
export const SenweaverCommandBar = ({ uri, editor }: SenweaverCommandBarProps) => {
	...
	const SenweaverModelService = accessor.get('ISenweaverModelService')
	...
	const _upKeybinding = keybindingService.lookupKeybinding(SENWEAVER_GOTO_PREV_DIFF_ACTION_ID);
	const _downKeybinding = keybindingService.lookupKeybinding(SENWEAVER_GOTO_NEXT_DIFF_ACTION_ID);
	const _leftKeybinding = keybindingService.lookupKeybinding(SENWEAVER_GOTO_PREV_URI_ACTION_ID);
	const _rightKeybinding = keybindingService.lookupKeybinding(SENWEAVER_GOTO_NEXT_URI_ACTION_ID);
	const _acceptFileKeybinding = keybindingService.lookupKeybinding(SENWEAVER_ACCEPT_FILE_ACTION_ID);
	const _rejectFileKeybinding = keybindingService.lookupKeybinding(SENWEAVER_REJECT_FILE_ACTION_ID);
	const _acceptAllKeybinding = keybindingService.lookupKeybinding(SENWEAVER_ACCEPT_ALL_DIFFS_ACTION_ID);
	const _rejectAllKeybinding = keybindingService.lookupKeybinding(SENWEAVER_REJECT_ALL_DIFFS_ACTION_ID);
	...
	if (currFileIdx === null) {
		return (
			<div className="pointer-events-auto">
				<div className="flex bg-senweaver-bg-2 shadow-md border border-senweaver-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:border-r [&>*]:border-senweaver-border-2 [&>*:last-child]:border-r-0">
		...
		<div className="pointer-events-auto">
			...
			{showAcceptRejectAllButtons && showAcceptRejectAll && (
				<div className="flex justify-end mb-1">
					<div className="inline-flex bg-senweaver-bg-2 rounded shadow-md border border-senweaver-border-2 overflow-hidden">
						<div className="flex items-center [&>*]:border-r [&>*]:border-senweaver-border-2 [&>*:last-child]:border-r-0">
			...
			<div className="flex items-center bg-senweaver-bg-2 rounded shadow-md border border-senweaver-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:px-3 [&>*]:border-r [&>*]:border-senweaver-border-2 [&>*:last-child]:border-r-0">
