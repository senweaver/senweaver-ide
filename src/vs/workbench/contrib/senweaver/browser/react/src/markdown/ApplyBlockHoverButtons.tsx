...
import { isFeatureNameDisabled } from '../../../../common/senweaverSettingsTypes.js'
import { URI } from '../../../../../../../base/common/uri.js'
import { FileSymlink, LucideIcon, RotateCw, Terminal } from 'lucide-react'
import { Check, X, Square, Copy, Play, } from 'lucide-react'
import { getBasename, ListableToolItem, senweaverOpenFileFn, ToolChildrenWrapper } from '../sidebar-tsx/SidebarChat.js'
...
export const IconShell1 = ({ onClick, Icon, disabled, className, ...props }: IconButtonProps & React.ButtonHTMLAttributes<HTMLButtonElement>) => {
	...
	return <button
		...
		// border border-senweaver-border-1 rounded
		className={`
		size-[18px]
		p-[2px]
		flex items-center justify-center
		text-sm text-senweaver-fg-3
...
export const JumpToFileButton = ({ uri, ...props }: { uri: URI | 'current' } & React.ButtonHTMLAttributes<HTMLButtonElement>) => {
	...
	const jumpToFileButton = uri !== 'current' && (
		<IconShell1
			Icon={FileSymlink}
			onClick={() => {
				senweaverOpenFileFn(uri, accessor)
			}}
...
export const useApplyStreamState = ({ applyBoxId }: { applyBoxId: string }) => {
	const accessor = useAccessor()
	const senweaverCommandBarService = accessor.get('ISenweaverCommandBarService')

	const getStreamState = useCallback(() => {
		const uri = getUriBeingApplied(applyBoxId)
		if (!uri) return 'idle-no-changes'
		return senweaverCommandBarService.getStreamState(uri)
	}, [senweaverCommandBarService, applyBoxId])
...
type IndicatorColor = 'green' | 'orange' | 'dark' | 'yellow' | 'blue' | null
export const StatusIndicator = ({ indicatorColor, title, className, ...props }: { indicatorColor: IndicatorColor, title?: React.ReactNode, className?: string } & React.HTMLAttributes<HTMLDivElement>) => {
	// Blue spinning indicator for "running" state
	if (indicatorColor === 'blue') {
		return (
			<div className={`flex flex-row text-senweaver-fg-3 text-xs items-center gap-1.5 ${className}`} {...props}>
				{title && <span className='opacity-80'>{title}</span>}
				<div
					className="size-3 rounded-full border-2 border-sky-400 border-t-transparent animate-spin"
					style={{
						boxShadow: '0 0 6px 0px rgba(56, 189, 248, 0.6)',
					}}
				/>
			</div>
		);
		...
		<div className={`flex flex-row text-senweaver-fg-3 text-xs items-center gap-1.5 ${className}`} {...props}>
			...
			<div
				className={` size-1.5 rounded-full border
					${indicatorColor === 'dark' ? 'bg-[rgba(0,0,0,0)] border-senweaver-border-1' :
						indicatorColor === 'orange' ? 'bg-orange-500 border-orange-500 shadow-[0_0_4px_0px_rgba(234,88,12,0.6)]' :
							indicatorColor === 'green' ? 'bg-green-500 border-green-500 shadow-[0_0_4px_0px_rgba(22,163,74,0.6)]' :
								indicatorColor === 'yellow' ? 'bg-yellow-500 border-yellow-500 shadow-[0_0_4px_0px_rgba(22,163,74,0.6)]' :
									'bg-senweaver-border-1 border-senweaver-border-1'
				}
			`}/>
		</div>
	);
...
const tooltipPropsForApplyBlock = ({ tooltipName, color = undefined, position = 'top', offset = undefined }: { tooltipName: string, color?: IndicatorColor, position?: PlacesType, offset?: number }) => ({
	'data-tooltip-id': color === 'orange' ? `senweaver-tooltip-orange` : color === 'green' ? 'senweaver-tooltip-green' : 'senweaver-tooltip',
...
export const useEditToolStreamState = ({ applyBoxId, uri }: { applyBoxId: string, uri: URI }) => {
	const accessor = useAccessor()
	const senweaverCommandBarService = accessor.get('ISenweaverCommandBarService')
	const [streamState, setStreamState] = useState(senweaverCommandBarService.getStreamState(uri))
	// listen for stream updates on this box
	useCommandBarURIListener(useCallback((uri_) => {
		const shouldUpdate = uri.fsPath === uri_.fsPath
		if (shouldUpdate) { setStreamState(senweaverCommandBarService.getStreamState(uri)) }
	}, [senweaverCommandBarService, applyBoxId, uri]))
...
const ApplyButtonsForEdit = ({
	...
	const onClickSubmit = useCallback(async () => {
		...
		if (!applyDonePromise) {
			notificationService.info(`SenWeaver Error: We couldn't run Apply here. ${uri === 'current' ? 'This Apply block wants to run on the current file, but you might not have a file open.' : `This Apply block wants to run on ${uri.fsPath}, but it might not exist.`}`)
		}
		...
		applyDonePromise?.catch(e => {
			...
			notificationService.info(`SenWeaver Error: There was a problem running Apply: ${e}.`)
	...
	if (currStreamState === 'idle-has-changes') {
		return <div className="flex items-center gap-0.5 bg-[#1e1e1e] rounded-full p-0.5">
			<button
				onClick={(e) => {
					e.preventDefault();
					e.stopPropagation();
				...
				className="flex items-center gap-1 px-2 py-0.5 text-xs font-medium bg-[#2d2d30] hover:bg-[#3d3d40] text-green-400 rounded-full transition-colors"
				{...tooltipPropsForApplyBlock({ tooltipName: 'Accept' })}
			>
				<Check size={12} />
				<span>Accept</span>
			</button>
			<button
				...
				className="flex items-center gap-1 px-2 py-0.5 text-xs font-medium bg-[#2d2d30] hover:bg-[#3d3d40] text-gray-400 rounded-full transition-colors"
				{...tooltipPropsForApplyBlock({ tooltipName: 'Reject' })}
			>
				<X size={12} />
				<span>Reject</span>
			</button>
...
export const BlockCodeApplyWrapper = ({
	...
	return <div className='border border-senweaver-border-3 rounded overflow-hidden bg-senweaver-bg-3 my-1'>
		{/* header */}
		<div className=" select-none flex justify-between items-center py-1 px-2 border-b border-senweaver-border-3 cursor-default">
			<div className="flex items-center">
				<StatusIndicatorForApplyButton uri={uri} applyBoxId={applyBoxId} />
				<span className="text-[13px] font-light text-senweaver-fg-3">
					{name}
				</span>
			</div>
