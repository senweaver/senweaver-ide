...
import { mountSenweaverSelectionHelper } from './react/out/senweaver-editor-widgets-tsx/index.js';
...
export type SenweaverSelectionHelperProps = {
...
export class SelectionHelperContribution extends Disposable implements IEditorContribution, IOverlayWidget {
	public static readonly ID = 'editor.contrib.SenweaverSelectionHelper';
	// react
	...
	constructor(
		...
		@ISenweaverSettingsService private readonly _SenweaverSettingsService: ISenweaverSettingsService
	) {
		super();
		...
		this._instantiationService.invokeFunction(accessor => {
			...
			const res = mountSenweaverSelectionHelper(content, accessor);
			if (!res) return;
	...
	private _showHelperForSelection(selection: Selection): void {
		...
		// rerender
		const enabled = this._SenweaverSettingsService.state.globalSettings.showInlineSuggestions
		...
		if (enabled) {
			this._rerender({ rerenderKey: this._rerenderKey } satisfies SenweaverSelectionHelperProps)
