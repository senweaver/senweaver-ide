...
import { SENWEAVER_ACCEPT_DIFF_ACTION_ID, SENWEAVER_REJECT_DIFF_ACTION_ID } from './actionIDs.js';
...
import { acceptBg, acceptBorder, acceptTextColor, buttonFontSize, rejectBg, rejectBorder, rejectTextColor } from '../common/helpers/colors.js';
import { DiffArea, Diff, CtrlKZone, SenweaverFileSnapshot, DiffAreaSnapshotEntry, diffAreaSnapshotKeys, DiffZone, TrackingZone, ComputedDiff } from '../common/editCodeServiceTypes.js';
...
// Helper function to normalize whitespace (collapse multiple spaces/tabs to single space)
const normalizeWhitespace = (str: string): string => {
	return str.split('\n').map(line => line.trim().replace(/\s+/g, ' ')).join('\n');
}

// Helper function to trim each line
const trimEachLine = (str: string): string => {
	return str.split('\n').map(line => line.trim()).join('\n');
}

// Helper function for fuzzy line matching - finds best match using line-by-line comparison
const fuzzyLineMatch = (searchLines: string[], fileLines: string[], startIdx: number = 0): number => {
	const searchNormalized = searchLines.map(l => l.trim().toLowerCase());
	const numSearchLines = searchNormalized.length;

	for (let i = startIdx; i <= fileLines.length - numSearchLines; i++) {
		let matches = 0;
		for (let j = 0; j < numSearchLines; j++) {
			const fileLine = fileLines[i + j].trim().toLowerCase();
			const searchLine = searchNormalized[j];
			// Check if lines are similar (exact match, or one contains the other, or very similar)
			if (fileLine === searchLine ||
				(searchLine.length > 10 && fileLine.includes(searchLine)) ||
				(searchLine.length > 10 && searchLine.includes(fileLine)) ||
				(searchLine.length > 5 && fileLine.replace(/\s/g, '') === searchLine.replace(/\s/g, ''))) {
				matches++;
			}
		}
		// If more than 80% of lines match, consider it a match
		if (matches >= numSearchLines * 0.8) {
			return i;
		}
	}
	return -1;
...
const findTextInCode = (text: string, fileContents: string, canFallbackToRemoveWhitespace: boolean, opts: { startingAtLine?: number, returnType: 'lines' }) => {
	...
	const returnAnsFromLine = (startLine: number) => {
		const numLines = numLinesOfStr(text)
		const endLine = startLine + numLines - 1
		return [startLine, endLine] as const
	}
	...
	const startingAtLine = opts?.startingAtLine ?? 0
	...
	// Strategy 2: Trim each line and match
	const trimmedText = trimEachLine(text)
	const trimmedFileContents = trimEachLine(fileContents)
	idx = trimmedFileContents.indexOf(trimmedText, startingAtLineIdx(trimmedFileContents))
	if (idx !== -1) {
		const lastIdx = trimmedFileContents.lastIndexOf(trimmedText)
		if (lastIdx !== idx) return 'Not unique' as const
		return returnAns(trimmedFileContents, idx)
	}

	// Strategy 3: Normalize whitespace and match
	const normalizedText = normalizeWhitespace(text)
	const normalizedFileContents = normalizeWhitespace(fileContents)
	...
	// Strategy 4: Remove all whitespace except newlines
	const noWhitespaceText = removeWhitespaceExceptNewlines(text)
	const noWhitespaceFileContents = removeWhitespaceExceptNewlines(fileContents)
	idx = noWhitespaceFileContents.indexOf(noWhitespaceText, startingAtLineIdx(noWhitespaceFileContents))
	if (idx !== -1) {
		const lastIdx = noWhitespaceFileContents.lastIndexOf(noWhitespaceText)
		if (lastIdx !== idx) return 'Not unique' as const
		return returnAns(noWhitespaceFileContents, idx)
	}

	// Strategy 5: Fuzzy line-by-line matching
	const searchLines = text.split('\n').filter(l => l.trim().length > 0)
	const fileLines = fileContents.split('\n')
	if (searchLines.length >= 2) {
		const lineIdx = fuzzyLineMatch(searchLines, fileLines, startingAtLine)
		if (lineIdx !== -1) {
			// Verify uniqueness by checking if there's another match
			const secondMatch = fuzzyLineMatch(searchLines, fileLines, lineIdx + 1)
			if (secondMatch !== -1) return 'Not unique' as const
			return returnAnsFromLine(lineIdx + 1) // Convert to 1-indexed
		}
	}
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	constructor(
		...
		const initializeModel = async (model: ITextModel) => {
			...
			await this._senweaverModelService.initializeModel(model.uri)
	...
	// RAF handles for batched user content change processing
	private _pendingUserChangeRAF = new Map<string, number>()

	private _onUserChangeContent(uri: URI, e: IModelContentChangedEvent) {
		const key = uri.fsPath

		// Immediately realign diff areas (necessary for correct cursor position)
		for (const change of e.changes) {
			this._realignAllDiffAreasLines(uri, change.text, change.range)
		}

		// Batch the rest of the processing using RAF to avoid blocking mouse/keyboard
		const existingRAF = this._pendingUserChangeRAF.get(key)
		if (existingRAF) {
			cancelAnimationFrame(existingRAF)
		}

		const rafId = requestAnimationFrame(() => {
			this._pendingUserChangeRAF.delete(key)
			this._processUserChangeContent(uri)
		})
		this._pendingUserChangeRAF.set(key, rafId)
	}

	private _processUserChangeContent(uri: URI) {
	...
	// 		message: `SenWeaver Error: ${e.message}`,
	// 		actions: {
	// 			secondary: [{
	// 				id: 'senweaver.onerror.opensettings',
	// 				enabled: true,
	// 				label: `Open SenWeaver's settings`,
	// 				tooltip: '',
	// 				class: undefined,
	// 				run: () => { this._commandService.executeCommand(SENWEAVER_OPEN_SETTINGS_ACTION_ID) }
	// 			}]
	// 		},
	// 		source: details ? `(Hold ${isMacintosh ? 'Option' : 'Alt'} to hover) - ${details}\n\nIf this persists, feel free to [report](https://github.com/SenweaverEditor/void/issues/new) it.` : undefined
	// 	})
	// }
		...
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] || []) {
			...
			if (diffArea.type === 'DiffZone') {
				...
				if (diffArea._streamState.isStreaming) {
					// sweepLine ... sweepLine
					const fn1 = this._addLineDecoration(model, diffArea._streamState.line, diffArea._streamState.line, 'senweaver-sweepIdxBG')
					// sweepLine+1 ... endLine
					const fn2 = diffArea._streamState.line + 1 <= diffArea.endLine ?
						this._addLineDecoration(model, diffArea._streamState.line + 1, diffArea.endLine, 'senweaver-sweepBG')
						: null
				...
				const fn = this._addLineDecoration(model, diffArea.startLine, diffArea.endLine, 'senweaver-highlightBG')
	...
	private _addDiffStylesToURI = (uri: URI, diff: Diff) => {
		...
		if (type !== 'deletion') {
			const fn = this._addLineDecoration(model, diff.startLine, diff.endLine, 'senweaver-greenBG', {
		...
		if (type !== 'insertion') {
			const consistentZoneId = this._consistentItemService.addConsistentItemToURI({
				...
				fn: (editor) => {
					...
					const domNode = document.createElement('div');
					domNode.className = 'senweaver-redBG'
		...
		if (diffZone.type === 'DiffZone' && !diffZone._streamState.isStreaming) {
			...
			const consistentWidgetId = this._consistentItemService.addConsistentItemToURI({
				...
				fn: (editor) => {
					...
					else { throw new Error('SenWeaver 1') }
	...
	private _getCurrentSenweaverFileSnapshot = (uri: URI): SenweaverFileSnapshot => {
		const { model } = this._senweaverModelService.getModel(uri)
	...
	private _restoreSenweaverFileSnapshot = async (uri: URI, snapshot: SenweaverFileSnapshot) => {
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => void }) {
		const beforeSnapshot: SenweaverFileSnapshot = this._getCurrentSenweaverFileSnapshot(uri)
		let afterSnapshot: SenweaverFileSnapshot | null = null

		const elt: IUndoRedoElement = {
			type: UndoRedoElementType.Resource,
			resource: uri,
			label: 'SenWeaver Agent',
			code: 'undoredo.editCode',
			undo: async () => { opts?.onWillUndo?.(); await this._restoreSenweaverFileSnapshot(uri, beforeSnapshot) },
			redo: async () => { if (afterSnapshot) await this._restoreSenweaverFileSnapshot(uri, afterSnapshot) }
		}
		this._undoRedoService.pushElement(elt)

		const onFinishEdit = async () => {
			afterSnapshot = this._getCurrentSenweaverFileSnapshot(uri)
			await this._senweaverModelService.saveModel(uri)
		}
		return { onFinishEdit }
	}


	public getSenweaverFileSnapshot(uri: URI) {
		return this._getCurrentSenweaverFileSnapshot(uri)
	}


	public restoreSenweaverFileSnapshot(uri: URI, snapshot: SenweaverFileSnapshot): Promise<void> {
		return this._restoreSenweaverFileSnapshot(uri, snapshot)
	}
	...
	// Batch fire events to avoid multiple RAF calls
	private _pendingDiffEvents = new Map<string, { uri: URI, diffareaids: Set<number> }>()
	private _pendingDiffEventsRAF: number | null = null

	private _fireChangeDiffsIfNotStreaming(uri: URI) {
		const key = uri.fsPath
		let pending = this._pendingDiffEvents.get(key)
		if (!pending) {
			pending = { uri, diffareaids: new Set() }
			this._pendingDiffEvents.set(key, pending)
		}
		...
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] || []) {
			...
			if (!diffArea._streamState.isStreaming) {
				pending.diffareaids.add(diffArea.diffareaid)
			}
		}

		// Batch fire all pending events in next animation frame
		if (!this._pendingDiffEventsRAF) {
			this._pendingDiffEventsRAF = requestAnimationFrame(() => {
				this._pendingDiffEventsRAF = null
				for (const [, { uri, diffareaids }] of this._pendingDiffEvents) {
					for (const diffareaid of diffareaids) {
						this._onDidChangeDiffsInDiffZoneNotStreaming.fire({ uri, diffareaid })
					}
				}
				this._pendingDiffEvents.clear()
			})
		}
	}


	// Throttle mechanism for _refreshStylesAndDiffsInURI to prevent UI blocking
	private _pendingRefreshURIs = new Map<string, ReturnType<typeof requestAnimationFrame>>()

	private _refreshStylesAndDiffsInURI(uri: URI) {
		const key = uri.fsPath

		// Cancel any pending refresh for this URI
		const pendingFrame = this._pendingRefreshURIs.get(key)
		if (pendingFrame) {
		...
		const frameId = requestAnimationFrame(() => {
			this._pendingRefreshURIs.delete(key)

			// Perform the actual refresh work
			this._doRefreshStylesAndDiffsInURI(uri)
		})

		this._pendingRefreshURIs.set(key, frameId)
	}

	private _doRefreshStylesAndDiffsInURI(uri: URI) {
	...
	private _writeStreamedDiffZoneLLMText(uri: URI, originalCode: string, llmTextSoFar: string, deltaText: string, latestMutable: StreamLocationMutable) {
		...
		if (!lastDiff) {
			...
			if (lastDiff.type === 'insertion' || lastDiff.type === 'edit')
				...
				throw new Error(`SenWeaver: diff.type not recognized on: ${lastDiff}`)
		}
	...
	public async callBeforeApplyOrEdit(opts: CallBeforeStartApplyingOpts) {
		...
		if (!uri) return
		await this._senweaverModelService.initializeModel(uri)
		await this._senweaverModelService.saveModel(uri) // save the URI
	}
	...
	private _initializeWriteoverStream(opts: StartApplyingOpts): [DiffZone, Promise<void>] | undefined {
		...
		if (from === 'ClickApply') {
			...
			throw new Error(`SenWeaver: diff.type not recognized on: ${from}`)
		}
	...
	private _instantlyApplySRBlocks(uri: URI, blocksStr: string) {
		...
		if (blocks.length === 0) {
			const preview = blocksStr.substring(0, 200)
			throw new Error(`No Search/Replace blocks were received! The search_replace_blocks parameter must contain properly formatted blocks starting with "<<<<<<< ORIGINAL". Received content preview: "${preview}${blocksStr.length > 200 ? '...' : ''}"`)
		}
		...
		const lineStartOffsets: number[] = new Array(modelStrLines.length)
		{
			let offset = 0
			for (let i = 0; i < modelStrLines.length; i++) {
				lineStartOffsets[i] = offset
				offset += modelStrLines[i].length
				if (i < modelStrLines.length - 1) {
					offset += 1 // '\n'
				}
			}
		}
		...
		for (const b of blocks) {
			...
			const origStart = lineStartOffsets[startLine] ?? 0

			// including endline at end
			const endLineStart = lineStartOffsets[endLine] ?? 0
			const endLineLen = modelStrLines[endLine]?.length ?? 0
			const origEnd = endLineStart + endLineLen - 1
			...
			while (shouldSendAnotherMessage) {
				...
				streamRequestIdRef.current = this._llmMessageService.sendLLMMessage({
					...
					onFinalMessage: async (params) => {
						...
						const blocks = extractSearchReplaceBlocks(fullText)
						if (blocks.length === 0) {
							this._notificationService.info(`SenWeaver: We ran Fast Apply, but the LLM didn't output any changes.`)
						}
						this._writeURIText(uri, originalFileCode, 'wholeFileRange', { shouldRealignDiffAreas: true })
	...
	// called on senweaver.acceptDiff
	public async acceptDiff({ diffid }: { diffid: number }) {
		...
		if (diff.type === 'deletion') {
			...
			throw new Error(`SenWeaver error: ${diff}.type not recognized`)
		}
	...
	// called on senweaver.rejectDiff
	...
	/**
	* 计算指定 URI 的所有 diff 统计信息
	* @param uri 文件 URI
	* @returns 包含新增和删除行数的统计对象
	*/
	public calculateDiffStats(uri: URI): { linesAdded: number, linesDeleted: number } {
		let linesAdded = 0;
		let linesDeleted = 0;
		...
		for (const diffareaid of diffareaids) {
			const diffArea = this.diffAreaOfId[diffareaid];
			if (!diffArea || diffArea.type !== 'DiffZone') continue;

			// 遍历此 DiffZone 的所有 diff
			for (const diffid in diffArea._diffOfId) {
				const diff = diffArea._diffOfId[diffid];

				if (diff.type === 'insertion') {
					// 纯插入：只计算新增行
					linesAdded += (diff.endLine - diff.startLine + 1);
				}
				else if (diff.type === 'deletion') {
					// 纯删除：只计算删除行
					linesDeleted += (diff.originalEndLine - diff.originalStartLine + 1);
				}
				else if (diff.type === 'edit') {
					// 编辑：同时计算新增和删除
					const oldLines = diff.originalEndLine - diff.originalStartLine + 1;
					const newLines = diff.endLine - diff.startLine + 1;
					linesAdded += newLines;
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		@ISenweaverCommandBarService private readonly _senweaverCommandBarService: ISenweaverCommandBarService,
		...
		const getAcceptRejectText = () => {
			const acceptKeybinding = this._keybindingService.lookupKeybinding(SENWEAVER_ACCEPT_DIFF_ACTION_ID);
			const rejectKeybinding = this._keybindingService.lookupKeybinding(SENWEAVER_REJECT_DIFF_ACTION_ID);
			...
			const commandBarStateAtUri = this._senweaverCommandBarService.stateOfURI[uri.fsPath];
		...
		// Style the container - add background wrapper like in image 3
		buttons.style.display = 'flex';
		buttons.style.position = 'absolute';
		buttons.style.gap = '2px';
		buttons.style.padding = '2px 4px';
		buttons.style.backgroundColor = '#1e1e1e'; // dark background container
		buttons.style.borderRadius = '9999px'; // fully rounded container
		buttons.style.zIndex = '100'; // 增加 z-index 确保在最上层
		buttons.style.transform = `translateY(${offsetLines * lineHeight}px)`;
		// 不设置 right，由 updateLeft 动态计算 left 位置
		buttons.style.pointerEvents = 'auto';
		buttons.style.whiteSpace = 'nowrap'; // 防止容器内换行


		// Style accept button - green text on dark background, fully rounded
		...
		acceptButton.style.color = acceptTextColor;
		acceptButton.style.fontSize = buttonFontSize;
		acceptButton.style.padding = '2px 8px';
		acceptButton.style.borderRadius = '9999px'; // fully rounded
		acceptButton.style.cursor = 'pointer';
		acceptButton.style.height = 'auto';
		acceptButton.style.pointerEvents = 'auto';
		acceptButton.style.transition = 'background-color 0.15s ease';
		acceptButton.style.whiteSpace = 'nowrap'; // 防止按钮内文字换行
		acceptButton.style.flexShrink = '0'; // 防止按钮被压缩
		acceptButton.onmouseenter = () => { acceptButton.style.backgroundColor = '#3d3d40'; };
		acceptButton.onmouseleave = () => { acceptButton.style.backgroundColor = acceptBg; };


		// Style reject button - gray text on dark background, fully rounded
		...
		rejectButton.style.color = rejectTextColor;
		rejectButton.style.fontSize = buttonFontSize;
		rejectButton.style.padding = '2px 8px';
		rejectButton.style.borderRadius = '9999px'; // fully rounded
		rejectButton.style.cursor = 'pointer';
		rejectButton.style.height = 'auto';
		rejectButton.style.pointerEvents = 'auto';
		rejectButton.style.transition = 'background-color 0.15s ease';
		rejectButton.style.whiteSpace = 'nowrap'; // 防止按钮内文字换行
		rejectButton.style.flexShrink = '0'; // 防止按钮被压缩
		rejectButton.onmouseenter = () => { rejectButton.style.backgroundColor = '#3d3d40'; };
		rejectButton.onmouseleave = () => { rejectButton.style.backgroundColor = rejectBg; };
		...
		const updateRight = () => {
			...
			// 计算按钮距离编辑器右边缘的距离（minimap + 滚动条 + 8px 间距）
			const minimapWidth = layoutInfo.minimap.minimapWidth;
			const verticalScrollbarWidth = layoutInfo.verticalScrollbarWidth;
			const rightPx = minimapWidth + verticalScrollbarWidth + 8;
			this._domNode.style.right = `${rightPx}px`;
			// 移除 left 设置
		...
		this._register(editor.onDidLayoutChange(e => { updateTop(); updateRight() }))


		// Listen for state changes in the command bar service
		this._register(this._senweaverCommandBarService.onDidChangeState(e => {
