...
import { Diff, DiffArea, SenweaverFileSnapshot } from '../common/editCodeServiceTypes.js';
...
export interface IEditCodeService {
	...
	// testDiffs(): void;
	getSenweaverFileSnapshot(uri: URI): SenweaverFileSnapshot;
	restoreSenweaverFileSnapshot(uri: URI, snapshot: SenweaverFileSnapshot): Promise<void>;

	calculateDiffStats(uri: URI): { linesAdded: number, linesDeleted: number };
}
