...
import { mountSenweaverCommandBar } from './react/out/senweaver-editor-widgets-tsx/index.js'
...
import { SENWEAVER_ACCEPT_DIFF_ACTION_ID, SENWEAVER_REJECT_DIFF_ACTION_ID, SENWEAVER_GOTO_NEXT_DIFF_ACTION_ID, SENWEAVER_GOTO_PREV_DIFF_ACTION_ID, SENWEAVER_GOTO_NEXT_URI_ACTION_ID, SENWEAVER_GOTO_PREV_URI_ACTION_ID, SENWEAVER_ACCEPT_FILE_ACTION_ID, SENWEAVER_REJECT_FILE_ACTION_ID, SENWEAVER_ACCEPT_ALL_DIFFS_ACTION_ID, SENWEAVER_REJECT_ALL_DIFFS_ACTION_ID } from './actionIDs.js';
...
export interface ISenweaverCommandBarService {
...
export const ISenweaverCommandBarService = createDecorator<ISenweaverCommandBarService>('SenweaverCommandBarService');
...
export class SenweaverCommandBarService extends Disposable implements ISenweaverCommandBarService {
	_serviceBrand: undefined;

	static readonly ID: 'senweaver.SenweaverCommandBarService'
	...
	// RAF-based throttle to prevent excessive event firing during rapid file changes
	private readonly _pendingStateChanges = new Map<string, number>();

	private _throttledFireStateChange(uri: URI) {
		const key = uri.fsPath;
		// Cancel any pending RAF for this URI
		const existing = this._pendingStateChanges.get(key);
		if (existing) {
			cancelAnimationFrame(existing);
		}
		// Use requestAnimationFrame to batch updates and avoid blocking main thread
		const rafId = requestAnimationFrame(() => {
			this._pendingStateChanges.delete(key);
			this._onDidChangeState.fire({ uri });
		});
		this._pendingStateChanges.set(key, rafId);
	}
		...
		this._register(this._editCodeService.onDidAddOrDeleteDiffZones(e => {
			for (const uri of this._listenToTheseURIs) {
				...
				if (diffZones.length === 0) {
					...
					this._throttledFireStateChange(uri)
	...
	async goToURIIdx(idx: number | null): Promise<void> {
		...
		const { model } = await this._senweaverModelService.getModelSafe(nextURI);
		if (!model) return;
...
registerSingleton(ISenweaverCommandBarService, SenweaverCommandBarService, InstantiationType.Eager);


export type SenweaverCommandBarProps = {
	uri: URI | null;
	editor: ICodeEditor;
}
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			const res = mountSenweaverCommandBar(root, accessor, { uri, editor } satisfies SenweaverCommandBarProps)
			if (!res) return
			this._register(toDisposable(() => res.dispose?.()))
			this._register(editor.onWillChangeModel((model) => {
				const uri = model.newModelUrl
				res.rerender({ uri, editor } satisfies SenweaverCommandBarProps)
			}))
		})
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: SENWEAVER_ACCEPT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('SenweaverAcceptDiffAction', 'SenWeaver: Accept Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: SENWEAVER_REJECT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('SenweaverRejectDiffAction', 'SenWeaver: Reject Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: SENWEAVER_GOTO_NEXT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('senweaverGoToNextDiffAction', 'SenWeaver: Go to Next Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: SENWEAVER_GOTO_PREV_DIFF_ACTION_ID,
			f1: true,
			title: localize2('senweaverGoToPrevDiffAction', 'SenWeaver: Go to Previous Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: SENWEAVER_GOTO_NEXT_URI_ACTION_ID,
			f1: true,
			title: localize2('senweaverGoToNextUriAction', 'SenWeaver: Go to Next File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: SENWEAVER_GOTO_PREV_URI_ACTION_ID,
			f1: true,
			title: localize2('senweaverGoToPrevUriAction', 'SenWeaver: Go to Previous File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: SENWEAVER_ACCEPT_FILE_ACTION_ID,
			f1: true,
			title: localize2('SenweaverAcceptFileAction', 'SenWeaver: Accept All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: SENWEAVER_REJECT_FILE_ACTION_ID,
			f1: true,
			title: localize2('SenweaverRejectFileAction', 'SenWeaver: Reject All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: SENWEAVER_ACCEPT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('SenweaverAcceptAllDiffsAction', 'SenWeaver: Accept All Diffs in All Files'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: SENWEAVER_REJECT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('SenweaverRejectAllDiffsAction', 'SenWeaver: Reject All Diffs in All Files'),
			keybinding: {
