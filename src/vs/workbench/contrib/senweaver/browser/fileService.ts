...
class FilePromptActionService extends Action2 {
	private static readonly SENWEAVER_COPY_FILE_PROMPT_ID = 'senweaver.copyfileprompt'

	constructor() {
		super({
			id: FilePromptActionService.SENWEAVER_COPY_FILE_PROMPT_ID,
			title: localize2('senweaverCopyPrompt', 'SenWeaver: Copy Prompt'),
			menu: [{
		...
		try {
			...
			if (stat.isFile) {
				m = await messageOfSelection({
					type: 'File',
					uri,
					language: (await senweaverModelService.getModelSafe(uri)).model?.getLanguageId() || '',
