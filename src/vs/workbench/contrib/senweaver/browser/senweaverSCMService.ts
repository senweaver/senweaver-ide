...
import { ISenweaverSCMService } from '../common/senweaverSCMTypes.js'
import { IMainProcessService } from '../../../../platform/ipc/common/mainProcessService.js'
import { ISenweaverSettingsService } from '../common/senweaverSettingsService.js'
import { IConvertToLLMMessageService } from './convertToLLMMessageService.js'
import { ILLMMessageService } from '../common/sendLLMMessageService.js'
import { ModelSelection, OverridesOfModel, ModelSelectionOptions } from '../common/senweaverSettingsTypes.js'
...
export const IGenerateCommitMessageService = createDecorator<IGenerateCommitMessageService>('senweaverGenerateCommitMessageService');

const loadingContextKey = 'SenweaverSCMGenerateCommitMessageLoading'
...
class GenerateCommitMessageService extends Disposable implements IGenerateCommitMessageService {
	...
	private SenweaverSCM: ISenweaverSCMService
	...
	constructor(
		...
		this.SenweaverSCM = ProxyChannel.toService<ISenweaverSCMService>(mainProcessService.getChannel('senweaver-channel-scm'))
	}
		...
		this.execute.trigger(async () => {
			...
			try {
				...
				const [stat, sampledDiffs, branch, log] = await Promise.all([
					this.SenweaverSCM.gitStat(path),
					this.SenweaverSCM.gitSampledDiffs(path),
					this.SenweaverSCM.gitBranch(path),
					this.SenweaverSCM.gitLog(path)
				])

				if (!this.isCurrentRequest(requestId)) { throw new CancellationError() }

				const modelSelection = this.SenweaverSettingsService.state.modelSelectionOfFeature['SCM'] ?? null
				const modelSelectionOptions = modelSelection ? this.SenweaverSettingsService.state.optionsOfModelSelection['SCM'][modelSelection?.providerName]?.[modelSelection.modelName] : undefined
				const overridesOfModel = this.SenweaverSettingsService.state.overridesOfModel
	...
	private sendLLMMessage(messages: LLMChatMessage[], separateSystemMessage: string, modelOptions: ModelOptions): Promise<string> {
		return new Promise((resolve, reject) => {
			...
			this.llmRequestId = this.llmMessageService.sendLLMMessage({
				...
				logging: { loggingName: 'SenweaverSCM - Commit Message' },
			})
		})
	}
	...
	private onError(error: any) {
		if (!isCancellationError(error)) {
			console.error(error)
			this.notificationService.error(localize2('senweaverFailedToGenerateCommitMessage', 'Failed to generate commit message.').value)
		}
	}
}
...
class GenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'senweaver.generateCommitMessageAction',
			title: localize2('senweaverCommitMessagePrompt', 'SenWeaver: Generate Commit Message'),
			icon: ThemeIcon.fromId('sparkle'),
			tooltip: localize2('senweaverCommitMessagePromptTooltip', 'SenWeaver: Generate Commit Message'),
			f1: true,
			menu: [{
...
class LoadingGenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'senweaver.loadingGenerateCommitMessageAction',
			title: { value: "SenWeaver: 取消提交消息生成", original: "SenWeaver: 取消提交消息生成" }, // hardcoded to prevent language switching issues
			icon: ThemeIcon.fromId('stop-circle'),
			tooltip: { value: "SenWeaver: 取消提交消息生成", original: "SenWeaver: 取消提交消息生成" }, // hardcoded to prevent language switching issues
