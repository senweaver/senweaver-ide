...
export class WebviewElement extends Disposable implements IWebview, WebviewFindDelegate {
	...
	constructor(
		...
		this._register(this.on('fatal-error', (e) => {
			// 忽略 ServiceWorker 相关的错误，因为在某些环境中（如 Electron）ServiceWorker 可能因数据库损坏等原因无法注册
			if (e.message && (e.message.includes('service worker') || e.message.includes('ServiceWorker'))) {
				console.warn('Webview ServiceWorker error (ignored):', e.message);
				return;
			}
