...
// 隐藏自定义布局菜单项 - Hidden by setting impossible condition
MenuRegistry.appendMenuItem(MenuId.LayoutControlMenu, {
	...
	when: ContextKeyExpr.false() // 设置为 false 以隐藏此菜单项
});
...
MenuRegistry.appendMenuItems([{
	...
	item: {
		...
		command: {
			id: ToggleSidebarPositionAction.ID,
			title: localize('move second sidebar left', "Move SenWeaver Side Bar Left")
		},
	...
	item: {
		...
		command: {
			id: ToggleSidebarPositionAction.ID,
			title: localize('move second sidebar right', "Move SenWeaver Side Bar Right")
		},
...
export class ToggleSidebarVisibilityAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('toggleSidebar', '主侧栏可见'),
			toggled: {
...
MenuRegistry.appendMenuItems([
	{
		...
		item: {
			...
			command: {
				id: ToggleSidebarVisibilityAction.ID,
				title: localize('compositePart.hideSideBarLabel', "关闭主侧栏"),
			},
		...
		item: {
			...
			command: {
				id: ToggleSidebarVisibilityAction.ID,
				title: localize('toggleSideBar', "主侧栏"),
...
export class ConfigureEditorTabsAction extends Action2 {
	...
	run(accessor: ServicesAccessor) {
		// Redirect to custom senweaver settings instead of native settings
		const commandService = accessor.get(ICommandService);
		return commandService.executeCommand('workbench.action.openSenweaverSettings');
	}
}
			...
			containerModel.visibleViewDescriptors.forEach(viewDescriptor => {
				if (viewDescriptor.canMoveView) {
					if (!hasAddedView) {
						results.push({
							type: 'separator',
							label: localize('secondarySideBarContainer', "SenWeaver Side Bar / {0}", containerModel.title)
						});
						hasAddedView = true;
					}
	...
	run(accessor: ServicesAccessor, viewId?: string): void {
		...
		if (!(isViewSolo && currentLocation === ViewContainerLocation.AuxiliaryBar)) {
			items.push({
				...
				label: localize('moveFocusedView.newContainerInSidePanel', "New SenWeaver Side Bar Entry")
			});
		}
		...
		items.push({
			type: 'separator',
			label: localize('secondarySideBar', "SenWeaver Side Bar")
		});
...
ToggleVisibilityActions.push(...[
	...
	CreateToggleLayoutItem(ToggleAuxiliaryBarAction.ID, AuxiliaryBarVisibleContext, localize('secondarySideBar', "SenWeaver Side Bar"), { whenA: ContextKeyExpr.equals('config.workbench.sideBar.location', 'left'), iconA: panelRightIcon, iconB: panelLeftIcon }),
			...
			menu: [
				{
					id: MenuId.LayoutControlMenuSubmenu,
					group: 'z_end',
					when: ContextKeyExpr.false() // 隐藏自定义布局菜单项
				},
				{
					id: MenuId.LayoutControlMenu,
					when: ContextKeyExpr.false(), // 隐藏自定义布局菜单项
