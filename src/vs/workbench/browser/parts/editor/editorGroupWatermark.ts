...
/* eslint-disable */ // Senweaver
import { SENWEAVER_CTRL_K_ACTION_ID, SENWEAVER_CTRL_L_ACTION_ID } from '../../../contrib/senweaver/browser/actionIDs.js';
...
export class EditorGroupWatermark extends Disposable {
	...
	private render(): void {
		...
		this.clear();
		const senweaverIconBox = append(this.shortcuts, $('.watermark-box'));
		...
		const update = async () => {
			...
			// Senweaver - if the workbench is empty, show open
			if (this.contextService.getWorkbenchState() === WorkbenchState.EMPTY) {
				...
				senweaverIconBox.appendChild(buttonContainer);
				...
				// show them Senweaver keybindings
				const keys = this.keybindingService.lookupKeybinding(SENWEAVER_CTRL_L_ACTION_ID);
				const dl = append(senweaverIconBox, $('dl'));
				...
				const keys2 = this.keybindingService.lookupKeybinding(SENWEAVER_CTRL_K_ACTION_ID);
				const dl2 = append(senweaverIconBox, $('dl'));
