...
export class NativeDialogHandler extends AbstractDialogHandler {
	...
	async about(): Promise<void> {
		...
		const detailString = (useAgo: boolean): string => {
			return `VSCode 版本: ${version}\nSenWeaver 版本: ${this.productService.SenWeaverVersion || 'Unknown'}\n日期: ${this.productService.date ? `${this.productService.date}${useAgo ? ' (' + fromNow(new Date(this.productService.date), true) + ')' : ''}` : 'Unknown'}\nChromium: ${process.versions['chrome']}\nNode.js: ${process.versions['node']}\nOS: ${osProps.type} ${osProps.arch} ${osProps.release}${isLinuxSnap ? ' snap' : ''}`;
		};
