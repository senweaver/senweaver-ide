...
import { ISenweaverUpdateService } from '../../workbench/contrib/senweaver/common/senweaverUpdateService.js';
import { MetricsMainService } from '../../workbench/contrib/senweaver/electron-main/metricsMainService.js';
import { SenweaverMainUpdateService } from '../../workbench/contrib/senweaver/electron-main/senweaverUpdateMainService.js';
import { LLMMessageChannel } from '../../workbench/contrib/senweaver/electron-main/sendLLMMessageChannel.js';
...
export class CodeApplication extends Disposable {
	...
	private configureSession(): void {
		...
		//#region Remove X-Frame-Options for built-in browser

		// 移除 X-Frame-Options 响应头，允许内置浏览器在 iframe/webview 中显示外部页面
		// 这对于局域网开发服务器特别重要，因为它们通常会设置 X-Frame-Options: sameorigin
		session.defaultSession.webRequest.onHeadersReceived({ urls: ['http://*/*', 'https://*/*'] }, (details, callback) => {
			const responseHeaders = details.responseHeaders as Record<string, (string) | (string[])> | undefined;
			if (responseHeaders) {
				// 移除 X-Frame-Options 头（可能是大写或小写）
				delete responseHeaders['X-Frame-Options'];
				delete responseHeaders['x-frame-options'];
				// 同时移除 Content-Security-Policy 中的 frame-ancestors 指令可能造成的问题
				// 注意：这里只删除头，不修改 CSP 内容，因为 CSP 更复杂
			}
			return callback({ cancel: false, responseHeaders });
		});
	...
	private async initServices(machineId: string, sqmId: string, devDeviceId: string, sharedProcessReady: Promise<MessagePortClient>): Promise<IInstantiationService> {
		...
		// Senweaver main process services (required for services with a channel for comm between browser and electron-main (node))
		services.set(IMetricsService, new SyncDescriptor(MetricsMainService, undefined, false));
		services.set(ISenweaverUpdateService, new SyncDescriptor(SenweaverMainUpdateService, undefined, false));
		services.set(ISenweaverSCMService, new SyncDescriptor(SenweaverSCMService, undefined, false));
	...
	private initChannels(accessor: ServicesAccessor, mainProcessElectronServer: ElectronIPCServer, sharedProcessClient: Promise<MessagePortClient>): void {
		...
		// Senweaver - use loggerChannel as reference
		const metricsChannel = ProxyChannel.fromService(accessor.get(IMetricsService), disposables);
		mainProcessElectronServer.registerChannel('senweaver-channel-metrics', metricsChannel);

		const senweaverUpdatesChannel = ProxyChannel.fromService(accessor.get(ISenweaverUpdateService), disposables);
		mainProcessElectronServer.registerChannel('senweaver-channel-update', senweaverUpdatesChannel);

		const sendLLMMessageChannel = new LLMMessageChannel(accessor.get(IMetricsService));
		mainProcessElectronServer.registerChannel('senweaver-channel-llmMessage', sendLLMMessageChannel);

		// Senweaver added this
		const senweaverSCMChannel = ProxyChannel.fromService(accessor.get(ISenweaverSCMService), disposables);
		mainProcessElectronServer.registerChannel('senweaver-channel-scm', senweaverSCMChannel);
