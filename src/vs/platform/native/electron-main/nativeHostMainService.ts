...
export class NativeHostMainService extends Disposable implements INativeHostMainService {
	...
	async captureWebviewScreenshot(windowId: number | undefined, webContentsId: number): Promise<Uint8Array | undefined> {
		const { webContents } = await import('electron');
		const allWebContents = webContents.getAllWebContents();

		// Find webview by webContentsId
		const targetWebContents = allWebContents.find(wc => wc.id === webContentsId);
		if (targetWebContents) {
			const captured = await targetWebContents.capturePage();
			return captured?.toPNG();
		}
	...
	async writeClipboardImage(windowId: number | undefined, imageData: VSBuffer): Promise<void> {
		const { nativeImage } = await import('electron');
		const image = nativeImage.createFromBuffer(Buffer.from(imageData.buffer));
